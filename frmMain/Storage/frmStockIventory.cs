...
namespace frmMain.Storage
{
    public partial class frmStockIventory : Form
    {
        ...
        Decimal _totalIventoryValue = 0; //Để phục vụ cho tính toán tổng giá trị phiếu kiểm
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Decimal iventoryTotal = _totalIventoryValue + pdChangedValue; //Cập nhập Số tiền lệch của cả phiếu kiểm kho
                ...
                if(addedPd == null)
                {
                    ...
                    _totalIventoryValue = iventoryTotal;
                    //Nếu bị thừa tiền, ta thêm dấu "+" vào tổng giá trị phiếu kiểm kho
                    if(iventoryTotal > 0)
                    {
                        txtTotalIventory.Text = "+" + String.Format("{0:0,0.00 VNĐ}", iventoryTotal);
                    }
                    else
                    {
                        txtTotalIventory.Text = String.Format("{0:0,0.00 VNĐ}", iventoryTotal);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if(dialogResult == DialogResult.Yes)
                {
                    ...
                    if(deletedRow != null)
                    {
                        Decimal newTotal = _totalIventoryValue - func.stringToDecimal(deletedRow["SoTienMHLech"].ToString());
                        _totalIventoryValue = newTotal; //Lấy giá trị phiếu kiểm mới

                        deletedRow.Delete();

                        //Hiển thị tổng giá trị phiếu kiểm mới
                        txtTotalIventory.Text = String.Format("{0:0,0.00 VNĐ}", newTotal);

                        //Nếu trong DGV không còn dữ liệu thì không hiển thị tổng giá trị phiếu kiểm theo Format
                        if (tblDGV.Rows.Count == 0)
                        {
                        ...
                        ResetInput();
                    }                    //Cập nhập lại tổng tiền phiểu kiểm
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if(ProductTextBIsFilled(code, quantity) || CheckIfInputValid(quantity))
            {
                return;
            }
            else
            {
                ...
                else
                {
                    ...
                    Decimal newIventoryTotal = _totalIventoryValue + newChangedValue; //Cập nhập Số tiền lệch của cả phiếu kiểm kho
                    ...
                    //Nếu bị thừa tiền, ta thêm dấu "+" vào tổng giá trị phiếu kiểm kho
                    if (newIventoryTotal > 0)
                    {
                        txtTotalIventory.Text = "+" + String.Format("{0:0,0.00 VNĐ}", newIventoryTotal);
                    }
                    else
                    {
                        txtTotalIventory.Text = String.Format("{0:0,0.00 VNĐ}", newIventoryTotal);
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if(dialogResult == DialogResult.Yes)
                {
                    ...
                    iventoryRow["SoTienLech"] = _totalIventoryValue;
